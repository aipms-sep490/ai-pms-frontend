...
| Eligibility read | BE_AVAILABLE | F5 Team eligibility summary / Team member | `TeamDto.eligibility` from current/detail Team reads | `GET /teams/current`, `GET /teams/{id}` | Team -> `canRegister`, `rosterLocked`, `registrationPeriodId`, `policyVersion`, `reasons[]` | Backend team membership/visibility | Team; 204/current-none, 401/403/404 | Explicit mock only | There is no separate GET eligibility resource or persisted snapshot DTO. |
| Eligibility refresh | BE_AVAILABLE | F5 summary / Team leader authorized by actions | typed API + `refresh_eligibility` workflow action | `POST /teams/{id}/eligibility/refresh` | none -> Team | Backend leader, roster/window/policy rules | Team/reasons; 401/403/404/409/422 | Explicit mock only | F5 refreshes Team/actions after success; no production local calculation. |
| Eligibility reasons | BE_AVAILABLE | F5 Team summary / Team member | `TeamEligibilityDto`, Team scope, workflow actions | Team/current/actions routes | backend `canRegister`, reason codes; dynamic `MAJOR_MIN_MEMBERS:<majorId>` / `MAJOR_MAX_MEMBERS:<majorId>` are supported | Backend team scope/roster | reasons; 401/403/404 | Explicit mock only | F5 maps known codes, keeps unknown codes visible, and never decides PASS/FAIL locally. |
| Eligibility freshness | BE_CONTRACT_LIMITATION | F5 Team summary | no backend snapshot/freshness DTO | none | no `checkedAt`, member/scope version, `isCurrent`, or `isStale` | N/A | N/A | N/A | F5 does not invent stale/not-checked from timestamps or roster comparisons; it labels the current Team read and offers explicit refresh when action-authorized. |

### Project, review, and lifecycle

| Capability | Status | Consumer / actor | Current FE / BE | Expected method and route | Request / response | Authorization, scope, precondition | Success / errors / concurrency | Mock allowed / production fallback | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Project create/detail/update | BE_AVAILABLE | F6 registration form / Team leader | `projects.api`, `useProjectRegistration`; ProjectsController | `POST /projects`; `GET/PUT /projects/{id}` | title, description, objectives, problemStatement, expectedOutput, requiredMajorIds, domain, technologies, keywords; update also requires `concurrencyToken` | `canRegister=true` plus `create_project_draft`; edit action for an existing Project | Project; 400/403/404/409/422 | Explicit mock only | Create has no source field. F6 creates/saves separately and refetches Project after every success. |
| Project majors/scope | BE_AVAILABLE | F6 governed read-only display | Project `academicScope` when returned; otherwise Team scope | no F6 scope mutation | ProjectMode, LeadDepartment, PrimaryMajor and requirements are backend-derived | Project visibility | data; 403/404 | Explicit mock only | F6 never free-types Major, Department or ProjectMode. Required Major IDs are derived only from returned scope or existing Project majors. |
| Submit/resubmit | BE_AVAILABLE | F6 status/form / leader | typed project API | `POST /projects/{id}/submit`; `POST /projects/{id}/resubmit` | `{ concurrencyToken }` -> Project | respective `submit_project`/`resubmit_project` workflow action | Project; 403/409/422 | Explicit mock only | Resubmit calls its own endpoint. No automatic 409 retry or local status transition. |
| Lifecycle/history | BE_AVAILABLE | F6 status / authorized viewer | typed API | `GET /projects/{id}/history` | none -> `oldStatus`, `newStatus`, actor, reason, timestamp | Project visibility | data; 401/403/404 | Explicit mock only | F6 renders only backend history; latest `REVISION_REQUIRED` reason remains visible. |
