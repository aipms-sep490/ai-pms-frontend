...
| Authentication refresh | BE_AVAILABLE | F1 session / authenticated | BE endpoint exists; no typed FE adapter/orchestration | `POST /auth/refresh` | refresh-token contract -> refreshed session | Valid refresh token | session; 401/409/5xx | No silent mock in production | F1 must implement according to real DTO. |
| Authentication logout | BE_AVAILABLE | F1 session / authenticated | FE only removes local token; BE endpoint exists | `POST /auth/logout` | backend contract -> 204/response | Authenticated session | 204 success; 401 ignored only after local cleanup | No silent mock in production | F1 must call server and clear local state. |
