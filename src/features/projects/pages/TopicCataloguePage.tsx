...
export function TopicCataloguePage() {
  ...
  return <div className="mx-auto flex max-w-6xl flex-col gap-6 pb-12">
    ...
    <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><h2 className="text-2xl font-bold tracking-tight text-slate-900">Danh mục đề tài</h2><p className="mt-1 text-sm text-slate-600">Lọc do Backend thực hiện. Phạm vi học vụ xác thực: {verifiedMajor ? `${verifiedMajor.code} · ${verifiedMajor.name}` : 'chưa có'}.</p></div><button type="button" onClick={() => navigate('/project/source?source=STUDENT_PROPOSAL')} className="rounded-xl border border-blue-200 px-4 py-2.5 text-xs font-bold text-blue-700 hover:bg-blue-50">Đề xuất dự án mới</button></header>
    ...
    <TopicDetailDrawer topic={selectedTopic} isLoading={detailLoading} error={detailError} onClose={() => { setSelectedTopic(null); setDetailError(null) }} onStartFromTopic={(topic) => navigate(`/project/source?topicId=${topic.id}`)} />
  </div>
}
