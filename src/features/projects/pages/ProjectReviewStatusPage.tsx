...
export function ProjectReviewStatusPage() {
  ...
  return <div className="mx-auto flex max-w-4xl flex-col gap-6 pb-16">
    ...
    <section className={`rounded-2xl border p-5 ${rejected ? 'border-rose-200 bg-rose-50' : 'border-blue-200 bg-blue-50'}`}><p className={`text-[11px] font-bold uppercase tracking-wider ${rejected ? 'text-rose-700' : 'text-blue-700'}`}>{project.status}</p><h2 className="mt-1 text-lg font-bold text-slate-950">{state.title}</h2><p className="mt-1 text-sm text-slate-700">{state.detail}</p>{rejection?.reason && <p className="mt-3 rounded-lg border border-rose-200 bg-white/70 p-3 text-sm text-rose-900"><strong>Lý do:</strong> {rejection.reason}</p>}{registration.status === 'DRAFT' && registration.canEdit && <button type="button" onClick={() => navigate('/project/edit')} className="mt-4 rounded-lg bg-blue-600 px-3.5 py-2 text-xs font-bold text-white">Tiếp tục chỉnh sửa</button>}{registration.status === 'REVISIONREQUIRED' && registration.canEdit && <button type="button" onClick={() => navigate('/project/edit')} className="mt-4 rounded-lg bg-amber-600 px-3.5 py-2 text-xs font-bold text-white">Xem phản hồi và chỉnh sửa</button>}{rejected && registration.canCreate && <button type="button" onClick={() => navigate('/project/register')} className="mt-4 rounded-lg bg-rose-600 px-3.5 py-2 text-xs font-bold text-white">Đăng ký đề tài mới</button>}{registration.status === 'APPROVED' && registration.canContinueToSupervisor && <button type="button" onClick={() => navigate('/project/supervisor')} className="mt-4 rounded-lg bg-emerald-600 px-3.5 py-2 text-xs font-bold text-white">Tiếp tục đến Giảng viên hướng dẫn</button>}</section>
