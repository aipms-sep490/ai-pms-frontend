...
import type { TeamInvitationCandidateDto, TeamInvitationDto } from '../../../types/backend'
...
export function TeamInvitationsPanel({
  ...
  const [candidates, setCandidates] = useState<TeamInvitationCandidateDto[]>([])
  const [candidateError, setCandidateError] = useState<string | null>(null)

  useEffect(() => {
    if (!teamId || !isLeader || rosterLocked) return
    let cancelled = false
    const timer = window.setTimeout(() => {
      services.team.getInvitationCandidates(teamId, { search: search.trim() || undefined, pageSize: 20 })
        .then((result) => { if (!cancelled) { setCandidates(result.items); setCandidateError(null) } })
        .catch((reason: unknown) => {
          if (!cancelled) setCandidateError(reason instanceof Error ? reason.message : 'Không thể tải ứng viên.')
        })
    }, 250)
    return () => { cancelled = true; window.clearTimeout(timer) }
  }, [teamId, isLeader, rosterLocked, search])

  const handleSend = async (candidate: TeamInvitationCandidateDto) => {
    if (!onSendInvitation) return
    setActionLoadingId(candidate.userId)
    try {
      await onSendInvitation(candidate.userId, message.trim() || undefined)
      setCandidates((items) => items.map((item) => item.userId === candidate.userId
        ? { ...item, canInvite: false, invitationStatus: 'PENDING' }
        : item))
      setMessage('')
    } finally {
        ...
        <div className="p-5 flex flex-col gap-5">
          ...
          {isLeader && !rosterLocked && (
            <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-4 flex flex-col gap-3">
              ...
              {candidateError && <p className="text-xs font-medium text-rose-700">{candidateError}</p>}
                ...
                {candidates.length === 0 ? (
                  ...
                  <div key={candidate.userId} className="flex items-center justify-between gap-3 p-3">
                    ...
                    <button type="button" onClick={() => handleSend(candidate)}
                      disabled={!candidate.canInvite || actionLoadingId === candidate.userId}
                      className="shrink-0 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-bold text-white disabled:bg-slate-300">
                      {candidate.invitationStatus === 'PENDING' ? 'Đã mời' : actionLoadingId === candidate.userId ? 'Đang gửi...' : 'Gửi lời mời'}
                    </button>
                  </div>
                ))}
              </div>
                  ...
                  <div key={inv.id} className="p-3.5 flex items-center justify-between gap-4 hover:bg-slate-50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isLeader && inv.status === 'PENDING' && !rosterLocked && (
                        ...
                        >
                          {actionLoadingId === inv.id ? 'Đang hủy...' : 'Hủy lời mời'}
                        </button>
                      )}
                    </div>
                  </div>
